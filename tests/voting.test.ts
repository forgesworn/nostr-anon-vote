...
import { VOTING_KINDS } from '../src/constants.js';
import type {
  ElectionScale,
  ReVotePolicy,
  ElectionParams,
...
describe('voting protocol', () => {
  ...
  describe('createElection', () => {
    ...
    it('includes all required tags', async () => {
      ...
      expect(tagMap.get('L')).toEqual(['anon-vote']);
      expect(tagMap.get('l')).toEqual(['anon-vote']);
...
describe('security hardening', () => {
  it('verifyBallot handles malformed ring-sig JSON gracefully', () => {
    const ballot = {
      ...
      content: '',
      tags: [
        ['L', 'anon-vote'], ['d', 'test'], ['election', 'abc'],
    ...
    const election = {
      ...
      tags: [
        ['L', 'anon-vote'], ['d', 'eid'], ['title', 'T'], ['scale', 'organisational'],
...
describe('structural validation', () => {
  ...
  it('rejects election with wrong kind', () => {
    const bad = { kind: 30470, tags: [['L', 'anon-vote']], pubkey: 'a'.repeat(64), content: '', id: 'x', sig: 'y', created_at: 0 } as any;
    const result = validateElection(bad);
    expect(result.valid).toBe(false);
  });

  it('rejects election missing required tags', () => {
    const bad = { kind: 30482, tags: [['L', 'anon-vote']], pubkey: 'a'.repeat(64), content: '', id: 'x', sig: 'y', created_at: 0 } as any;
    const result = validateElection(bad);
    expect(result.valid).toBe(false);
