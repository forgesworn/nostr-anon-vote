...
import type {
  ...
  ElectionScale, ReVotePolicy,
} from './types.js';

// ---------------------------------------------------------------------------
// Election creation and parsing
// ---------------------------------------------------------------------------
...
export async function createElection(
  ...
  if (params.options.length < 2) {
    throw new VotingError('Election must have at least 2 options');
  }
  if (params.closes <= params.opens) {
    throw new VotingError('Election closing time must be after opening time');
  }
  if (params.tallyPubkeys.length < 1) {
    throw new VotingError('Election must have at least 1 tally pubkey');
  }
...
export function parseElection(event: NostrEvent): ParsedElection | null {
  ...
  const eligibleMinTier = (isNaN(rawMinTier) || rawMinTier < 1) ? 1 : rawMinTier;
...
export async function decryptBallotContent(
  ...
  if (encrypted.length < 120) {
    throw new ValidationError('Encrypted ballot content too short');
  }
...
export async function castBallot(
  ...
  if (!parsed) throw new VotingError('Invalid election event');
  ...
  if (!parsed.options.includes(selectedOption)) {
    throw new VotingError('Selected option is not valid for this election');
  }

  // Validate election is open
  const now = Math.floor(Date.now() / 1000);
  if (now < parsed.opens) {
    throw new VotingError('Election has not yet opened');
  }
  if (now >= parsed.closes) {
    throw new VotingError('Election has already closed');
  }
...
export function validateElection(event: NostrEvent): ValidationResult {
  ...
  if (!event.tags.some(t => t[0] === 'L' && t[1] === DEFAULT_LABEL)) {
    errors.push(`Missing protocol label (["L", "${DEFAULT_LABEL}"])`);
  }
